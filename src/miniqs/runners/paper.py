...
- Optional: ``ALPACA_SYMBOLS=BTC/USD`` (comma-separated), ``ALPACA_MAX_TICKS=500`` for bounded runs.
...
from src.miniqs.config.asset import is_asset_tradable_now, resolve_asset_config
...
from src.miniqs.execution.trading import run_trading_stream_listener
from src.miniqs.engine.event_bus import EventBus, EventDispatcher, FillEvent, MarketEvent, OrderEvent, SignalEvent
from src.miniqs.engine.feature import FeatureEngine
from src.miniqs.ui.live_dashboard import append_csv_row, append_jsonl, dashboard_row, write_snapshot
...
from src.miniqs.risk.engine import RiskConfig, check_risk
from src.miniqs.signals.strategies import FunctionStrategy, StrategyRegistry, default_strategy_registry, generate_weighted_signals, mean_reversion_signal, momentum_signal, volatility_breakout_signal
...
def _runtime_asset_config(runtime: AlpacaRuntime, control_state: Optional[Dict[str, Any]] = None):
    control_asset = (
        control_state.get("asset", {})
        if isinstance(control_state, dict) and isinstance(control_state.get("asset"), dict)
        else {}
    )
    merged_asset = {
        "symbol": str(control_asset.get("symbol") or runtime.asset.get("symbol") or runtime.primary),
        "asset_type": str(control_asset.get("asset_type") or runtime.asset.get("asset_type") or ""),
        "market_hours": control_asset.get("market_hours")
        if isinstance(control_asset.get("market_hours"), dict)
        else runtime.asset.get("market_hours"),
        "trading_fees": float(control_asset.get("trading_fees", runtime.asset.get("trading_fees", runtime.cfg.trading_fees))),
    }
    return resolve_asset_config(
        symbol=str(merged_asset["symbol"]),
        asset_type=str(merged_asset["asset_type"]) or None,
        market_hours=merged_asset["market_hours"] if isinstance(merged_asset["market_hours"], dict) else None,
        trading_fees=float(merged_asset["trading_fees"]),
    )


def _engage_kill_switch(runtime: AlpacaRuntime, *, reason: str) -> None:
    state = load_control_state()
    if state.get("kill_switch") and not state.get("trading_enabled", True):
        return
    state["kill_switch"] = True
    state["trading_enabled"] = False
    state["kill_switch_reason"] = reason
    state["kill_switch_at"] = datetime.now(timezone.utc).isoformat()
    save_control_state(state)
    runtime.logger.log_connection_event("risk", "kill_switch", reason)
    _log.error("[alpaca_runner] kill switch engaged: %s", reason)


def _reconcile_runtime_state(runtime: AlpacaRuntime) -> tuple[bool, str]:
    client = runtime.alpaca_exec.client
    account = client.get_account()
    broker_cash = float(account.get("cash", runtime.portfolio.cash))
    if abs(broker_cash - runtime.portfolio.cash) > max(50.0, 0.0025 * max(runtime.initial_equity, 1.0)):
        return False, f"cash_mismatch local={runtime.portfolio.cash:.2f} broker={broker_cash:.2f}"

    if hasattr(client, "get_positions"):
        try:
            positions = client.get_positions()
        except Exception as exc:  # noqa: BLE001
            return False, f"position_reconcile_failed error={exc}"
        broker_pos = 0.0
        symbol_upper = runtime.primary.upper()
        for row in positions if isinstance(positions, list) else []:
            if not isinstance(row, dict):
                continue
            if str(row.get("symbol", "")).upper() != symbol_upper:
                continue
            broker_pos = float(row.get("qty", 0.0) or 0.0)
            break
        if abs(broker_pos - runtime.portfolio.position_size) > 1e-8:
            return False, f"position_mismatch local={runtime.portfolio.position_size:.8f} broker={broker_pos:.8f}"
...
def _on_market_event(event: MarketEvent, bus: EventBus, runtime: AlpacaRuntime) -> None:
    ...
    asset_cfg = _runtime_asset_config(runtime)
    if not is_asset_tradable_now(asset_cfg, tick.timestamp):
        stage = "market_closed"
        ...
        _emit_heartbeat(
            ...
            metrics=metrics,
            detail="equity_market_closed",
            trace="asset=equity market_hours_gate=closed",
        )
        return
    ...
    if runtime.tick_counter % 50 == 0:
        try:
            ok, detail = _reconcile_runtime_state(runtime)
        except Exception as exc:  # noqa: BLE001
            ok, detail = False, f"reconcile_exception:{exc}"
        if not ok:
            runtime.reconciliation_failures += 1
            _write_brain_trace(
                runtime.cfg,
                {
                    "kind": "risk",
                    ...
                    "detail": detail,
                    "reconciliation_failures": runtime.reconciliation_failures,
                },
            )
            _engage_kill_switch(runtime, reason=f"reconciliation_failed:{detail}")
...
def _on_order_event(event: OrderEvent, bus: EventBus, runtime: AlpacaRuntime) -> None:
    ...
    control_state = load_control_state()
    if not bool(control_state.get("trading_enabled", True)) or bool(control_state.get("kill_switch", False)):
        runtime.logger.log_risk_block("trading_stopped_pre_submit", json.dumps(trade, default=str))
        return

    trade_ts = _parse_iso_utc(trade.get("timestamp"))
    submit_asset_cfg = _runtime_asset_config(runtime, control_state)
    if not is_asset_tradable_now(submit_asset_cfg, trade_ts):
        runtime.logger.log_risk_block("market_closed_pre_submit", json.dumps(trade, default=str))
        return

    try:
        result = runtime.alpaca_exec.execute_trade(trade)
        runtime.order_error_count = 0
    except Exception as exc:  # noqa: BLE001
        runtime.order_error_count += 1
        ...
        _write_brain_trace(
            runtime.cfg,
            {
                **base_trace,
                ...
                "trade": trade,
                "error": str(exc),
                "order_error_count": runtime.order_error_count,
            },
        )
        if runtime.order_error_count >= 3:
            _engage_kill_switch(runtime, reason=f"execution_error_burst:{runtime.order_error_count}")
        return
...
async def run_alpaca_paper_session(config: Optional[AlpacaConfig] = None) -> Dict[str, float]:
    ...
    startup_control = load_control_state()
    control_asset = startup_control.get("asset", {}) if isinstance(startup_control.get("asset"), dict) else {}
    if control_asset.get("symbol"):
        cfg.symbols = [str(control_asset.get("symbol")).upper()]
    if control_asset.get("asset_type"):
        cfg.asset_type = str(control_asset.get("asset_type"))
    if control_asset.get("market_hours") and isinstance(control_asset.get("market_hours"), dict):
        cfg.market_hours = dict(control_asset.get("market_hours"))
    if control_asset.get("trading_fees") is not None:
        cfg.trading_fees = float(control_asset.get("trading_fees"))
    if cfg.asset_type == "equity" and cfg.data_feed.startswith("crypto"):
        cfg.data_feed = "iex"
        cfg.data_ws_url = "wss://stream.data.alpaca.markets/v2/iex"
    if cfg.asset_type == "crypto" and not cfg.data_feed.startswith("crypto"):
        cfg.data_feed = "crypto/us"
        cfg.data_ws_url = "wss://stream.data.alpaca.markets/v1beta3/crypto/us"
    ...
    client = AlpacaPaperClient(
        ...
        cfg.api_secret_key,
        cfg.rest_base_url,
        trading_api_version=cfg.trading_api_version,
    )
    ...
    if cfg.sync_initial_cash_from_alpaca:
        try:
            ...
            _log.error("[alpaca_runner] startup account sync failed; refusing to trade: %s", exc)
            raise RuntimeError("startup account sync failed; trading aborted") from exc
