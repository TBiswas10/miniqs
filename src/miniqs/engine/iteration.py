...
class RegimeDetector:
    ...
    def detect(self, prices: List[float], rsi: float = 50.0, macd: float = 0.0, macd_signal: float = 0.0) -> MarketConditions:
        if len(prices) < 3:
...
class ExperimentLogger:
    ...
    def clear_all(self) -> None:
        """Wipe all existing experiment logs for a clean session start."""
        conn = self._connect()
        try:
            conn.execute("DELETE FROM experiments")
            conn.commit()
        except Exception as e:
            print(f"Experiment purge failed: {e}")
        finally:
            conn.close()
