...
def _alpaca_api_version(value: Any, default: str = "v2") -> str:
    normalized = str(value or default).strip().lower()
    return normalized if normalized in {"v2"} else default


def _default_data_ws_url(data_feed: str) -> str:
    feed = str(data_feed or "").strip().lower()
    if feed.startswith("crypto/"):
        return f"wss://stream.data.alpaca.markets/v1beta3/{feed}"
    if not feed:
        feed = "iex"
...
class AlpacaConfig:
    ...
    data_ws_url: str = "wss://stream.data.alpaca.markets/v1beta3/crypto/us"
    data_feed: str = "crypto/us"
    ...
    trade_size: float = 0.0025
    risk_per_trade: float = 0.003
    daily_loss_limit: float = 300.0
    confidence_threshold: float = 0.42
    evaluation_profile: str = "default"
    mr_threshold: float = 0.004
    mom_threshold: float = 0.0028
    vb_breakout_factor: float = 1.1
    feedback_trade_interval: int = 15
    ...
    order_poll_interval_seconds: float = 0.35
    execution_max_retries: int = 4
    execution_retry_backoff_seconds: float = 1.25
    ...
    def validate(self) -> None:
        """Validate that the configuration is safe for trading."""
        if not self.api_key_id or not self.api_secret_key:
            raise ValueError(
                "ALPACA_API_KEY_ID or ALPACA_API_SECRET_KEY is missing. "
                "Check your environment variables or .env file."
            )
        
        if not self.symbols:
            raise ValueError("At least one symbol must be configured (default: BTC/USD).")
        
        for symbol in self.symbols:
            if "/" not in symbol and self.asset_type == "crypto":
                # Binance/Alpaca Crypto usually needs "/" in the symbol name for our logic
                pass
    ...
    def from_file(cls, file_path: str | Path) -> "AlpacaConfig":
        ...
        symbols = root.get("symbols", ["BTC/USD"])
        ...
        resolved_asset = resolve_asset_config(
            symbol=symbol_list[0],
            asset_type=str(root.get("asset_type", "") or "").lower() or None,
            market_hours=root.get("market_hours") if isinstance(root.get("market_hours"), dict) else None,
            trading_fees=float(root.get("trading_fees")) if root.get("trading_fees") is not None else None,
        )

        data_feed = str(market_data.get("data_feed", root.get("data_feed", "crypto/us"))).strip().lower()
        if "data_feed" not in market_data and "data_feed" not in root:
            data_feed = "crypto/us" if resolved_asset.asset_type == "crypto" else "iex"
        data_ws_default = _default_data_ws_url(data_feed)
        ...
        return cls(
            ...
            trading_api_version=_alpaca_api_version(root.get("trading_api_version", "v2")),
            data_ws_url=str(market_data.get("data_ws_url", root.get("data_ws_url", data_ws_default))),
            data_feed=data_feed,
            ...
            market_hours=(
                {
                    "open": resolved_asset.market_hours.open,
                    "close": resolved_asset.market_hours.close,
                    "timezone": resolved_asset.market_hours.timezone,
                }
                if resolved_asset.market_hours
                else None
            ),
            trading_fees=float(root.get("trading_fees", resolved_asset.trading_fees)),
            ...
            min_tick_interval_seconds=float(market_data.get("min_tick_interval_seconds", root.get("min_tick_interval_seconds", 0.25))),
            ...
            max_position_size=float(risk.get("max_position_size", root.get("max_position_size", 0.03))),
            cooldown_seconds=int(risk.get("cooldown_seconds", root.get("cooldown_seconds", 12))),
            max_loss_per_session=float(risk.get("max_loss_per_session", root.get("max_loss_per_session", 300.0))),
            trade_size=float(risk.get("trade_size", root.get("trade_size", 0.0025))),
            risk_per_trade=float(risk.get("risk_per_trade", root.get("risk_per_trade", 0.003))),
            daily_loss_limit=float(risk.get("daily_loss_limit", root.get("daily_loss_limit", 300.0))),
            confidence_threshold=float(risk.get("confidence_threshold", root.get("confidence_threshold", 0.42))),
            evaluation_profile=str(strategy.get("evaluation_profile", root.get("evaluation_profile", "default"))),
            mr_threshold=float(strategy.get("mr_threshold", root.get("mr_threshold", 0.004))),
            mom_threshold=float(strategy.get("mom_threshold", root.get("mom_threshold", 0.0028))),
            vb_breakout_factor=float(strategy.get("vb_breakout_factor", root.get("vb_breakout_factor", 1.1))),
            feedback_trade_interval=int(strategy.get("feedback_trade_interval", root.get("feedback_trade_interval", 15))),
            ...
            portfolio_fee_rate=float(root.get("portfolio_fee_rate", resolved_asset.trading_fees)),
            order_fill_timeout_seconds=float(execution.get("order_fill_timeout_seconds", root.get("order_fill_timeout_seconds", 60.0))),
            order_poll_interval_seconds=float(execution.get("order_poll_interval_seconds", root.get("order_poll_interval_seconds", 0.35))),
            execution_max_retries=int(execution.get("max_retries", root.get("execution_max_retries", 4))),
            execution_retry_backoff_seconds=float(execution.get("retry_backoff_seconds", root.get("execution_retry_backoff_seconds", 1.25))),
    ...
    def from_env(cls, symbols: Optional[Sequence[str]] = None) -> "AlpacaConfig":
        ...
        env_asset_type = _env("ALPACA_ASSET_TYPE", "")
        resolved_asset = resolve_asset_config(
            symbol=sym_list[0],
            asset_type=env_asset_type or base.asset_type,
            market_hours=base.market_hours if isinstance(base.market_hours, dict) else None,
            trading_fees=base.trading_fees,
        )

        data_feed_default = "crypto/us" if resolved_asset.asset_type == "crypto" else "iex"
        data_feed = _env("ALPACA_DATA_FEED", base.data_feed or data_feed_default).lower()
        data_ws_env = _env("ALPACA_DATA_WS_URL", "")
        data_ws = data_ws_env or _default_data_ws_url(data_feed)
        ...
        return cls(
            ...
            trading_api_version=_alpaca_api_version(
                _env("ALPACA_TRADING_API_VERSION", base.trading_api_version),
                default=base.trading_api_version,
            ),
            data_ws_url=data_ws,
            data_feed=data_feed,
            ...
            market_hours=(
                ...
                if resolved_asset.market_hours
                else base.market_hours
            ),
            trading_fees=float(_env("ALPACA_TRADING_FEES", str(base.trading_fees))),
            ...
            portfolio_fee_rate=float(_env("ALPACA_PORTFOLIO_FEE_RATE", str(base.portfolio_fee_rate or resolved_asset.trading_fees))),
