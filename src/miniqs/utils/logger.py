...
class QuantLogger:
	...
	def vacuum(self) -> None:
		"""Compact the database to save space and maintain performance."""
		conn = self._connect()
		try:
			conn.execute("VACUUM")
			self.log_ws_event("system", "Database vacuumed and compacted.")
		except Exception as e:
			self.log_ws_event("system", f"Vacuum failed: {e}")
		finally:
			conn.close()

	def clear_all(self) -> None:
		"""Wipe all existing logs and traces for a clean session start."""
		conn = self._connect()
		try:
			cur = conn.cursor()
			tables = ["event_log", "signals", "trades", "portfolio_snapshots", 
					  "performance_metrics", "ws_events", "connection_events", 
					  "risk_blocks", "feedback_log"]
			for table in tables:
				cur.execute(f"DELETE FROM {table}")
			conn.commit()
			# Also truncate the JSONL file
			with open(self.jsonl_path, "w", encoding="utf-8") as handle:
				handle.truncate(0)
			self.log_ws_event("system", "Station data purged (Full Reset).")
		except Exception as e:
			print(f"Purge failed: {e}")
		finally:
			conn.close()

	def purge_old_data(self, hours: int = 24) -> None:
		"""Delete records older than the threshold to keep the DB lightweight."""
		conn = self._connect()
		try:
			cur = conn.cursor()
			# SQLite datetime comparison
			threshold = f"-{hours} hours"
			tables = ["event_log", "ws_events", "connection_events", "risk_blocks"]
			for table in tables:
				cur.execute(f"DELETE FROM {table} WHERE ts < datetime('now', ?)", (threshold,))
			conn.commit()
			self.log_ws_event("system", f"Pruned records older than {hours}h.")
		except Exception as e:
			self.log_ws_event("system", f"Pruning failed: {e}")
		finally:
			conn.close()
