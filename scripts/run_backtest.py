...
from src.miniqs.config.asset import AssetConfig, resolve_asset_config
from src.miniqs.data.data_feed import Tick
from src.miniqs.engine.event_bus import EventBus, EventDispatcher, ExperimentLogEvent, FillEvent, IterationEvent, MarketEvent, OrderEvent, SignalEvent
...
from src.miniqs.signals.strategies import StrategyRegistry, default_strategy_registry, generate_weighted_signals
from src.miniqs.signals.evaluator import emit_signal_event, evaluate_signals
...
def run_backtest(
	...
	cfg = config or {}
	asset = resolve_asset_config(
		symbol=str(cfg.get("symbol", "BTC/USD")),
		asset_type=str(cfg.get("asset_type", "") or "") or None,
	)
	...
	with tempfile.TemporaryDirectory() as tmp:
		...
		mr_threshold = float(cfg.get("mr_threshold", 0.004 if asset.asset_type == "crypto" else 0.003))
		mom_threshold = float(cfg.get("mom_threshold", 0.0028 if asset.asset_type == "crypto" else 0.002))
		vb_breakout_factor = float(cfg.get("vb_breakout_factor", 1.1 if asset.asset_type == "crypto" else 1.2))
		confidence_threshold = float(cfg.get("confidence_threshold", 0.42 if asset.asset_type == "crypto" else 0.6))
		trade_size = float(cfg.get("trade_size", 0.0025 if asset.asset_type == "crypto" else 1.0))
		max_position_size = float(cfg.get("max_position_size", 0.03 if asset.asset_type == "crypto" else 5.0))
		cooldown_seconds = int(cfg.get("cooldown_seconds", 12 if asset.asset_type == "crypto" else 5))
		max_loss_per_session = float(cfg.get("max_loss_per_session", 300.0 if asset.asset_type == "crypto" else 500.0))
		...
		for i, (p, ts) in enumerate(zip(prices, _build_backtest_timestamps(len(prices), asset))):
			tick = Tick(symbol=asset.symbol, price=float(p), timestamp=ts, volume=1.0)
...
def _build_backtest_timestamps(length: int, asset: AssetConfig) -> List[datetime]:
	if length <= 0:
		return []
	...
	if asset.asset_type == "crypto":
		return [now + (step * i) for i in range(length)]

	hours = asset.market_hours
	if hours is None:
		return [now + (step * i) for i in range(length)]

	tz = ZoneInfo(hours.timezone)
	open_hour, open_min = [int(x) for x in hours.open.split(":", 1)]
	close_hour, close_min = [int(x) for x in hours.close.split(":", 1)]

	def _next_open_local(dt_local: datetime) -> datetime:
		candidate = dt_local.replace(hour=open_hour, minute=open_min, second=0, microsecond=0)
		if dt_local.weekday() >= 5:
			days = 7 - dt_local.weekday()
			return (candidate + timedelta(days=days)).replace(hour=open_hour, minute=open_min, second=0, microsecond=0)
		if dt_local.hour * 60 + dt_local.minute < open_hour * 60 + open_min:
			return candidate
		if dt_local.hour * 60 + dt_local.minute >= close_hour * 60 + close_min:
			next_day = dt_local + timedelta(days=1)
			while next_day.weekday() >= 5:
				next_day = next_day + timedelta(days=1)
			return next_day.replace(hour=open_hour, minute=open_min, second=0, microsecond=0)
		return dt_local.replace(second=0, microsecond=0)

	local_cursor = _next_open_local(now.astimezone(tz))
	local_close_minutes = close_hour * 60 + close_min
	ts_list: List[datetime] = []

	for _ in range(length):
		ts_list.append(local_cursor.astimezone(timezone.utc))
		local_cursor = local_cursor + step
		if local_cursor.weekday() >= 5 or (local_cursor.hour * 60 + local_cursor.minute) >= local_close_minutes:
			local_cursor = _next_open_local(local_cursor)
